...
traveler_gender %>%
  ...
  labs(x = 'Gender', y = 'Proportion', 
       title = "Propotion of Male and Female Respondents", 
...
There's a question in the questionnaire which is "How important is it to you that making it safer and easier to bike by adding bike lanes". This helps us decide one aspect which people think causes the unsafety of riding bicycle in New York City. We plot it into a diverted bar chart. It shows that both males and females think adding bike lanes are really important, although with different degree. This indicates that the current number of bike lanes in NYC are still not enough and car accident may negatively influence the proportion of residents who would like to ride a bicycle to get around the city.
...
traveler_income %>%
  ...
  labs(x = 'Income', y = 'Proportion', 
       title = "Propotion of Respondents with Different Income", 
       caption = "Source: https://data.cityofnewyork.us/Transportation/Citywide-Mobility-Survey-Main-Survey/dd6w-hnq9") +
  theme_economist()+
  theme(axis.text.x = element_text(size = 8, angle = 0))
```
...
![New York City subway system map](image/subway_map.png)
...
age %>%
  ...
  labs(x = 'Age', y = 'Proportion', 
       title = "Propotion of Respondents with Different Age", 
       caption = "Source: https://data.cityofnewyork.us/Transportation/Citywide-Mobility-Survey-Main-Survey/dd6w-hnq9") +
  theme_economist() +
  theme(axis.text.x = element_text(size = 8, angle = 0), 
        legend.position = "right", 
...
Apart from basic socioeconomic factors, the situation of living and dwelling should also affect their preferences for traveling and types of transportation. We select variables concerning whether the respondent own their house or not, the cumulative years of living in New York City and the number of children as index of their general living condition and explore whether owning house, rasing children and living longer years in NYC will have an impact on their choices.
...
We expect children to be a distinguishing feature as rasing child would definitely change almost every aspects of an adult's life with traveling included. In all ways of transportation, people with two or more children take up highest proportion in the car category. It's reasonble because one can imagine the trouble of taking subway with two kids but it's also possible that the result coming from the reciporical effects of other variables. For example, those who live longer and own their house are also likely to buy a car and having more children in New York City. Young people will have significant disadvantages in terms of their wealth accumulation and years of living in the city. Further understanding of the phenomenon requires a deeper analysis to control the effect of other variables. 
...
ggplot(df_takeout %>% 
  ...
  geom_col() +
  labs(x = "Frequency of Receving Takeout Food", y = "Frequency",
       title = "The Distribution of Respondents in Frequency of Receving Takeout Food",
       caption = "Source: https://data.cityofnewyork.us/Transportation/Citywide-Mobility-Survey-Main-Survey/dd6w-hnq9") +
  theme_economist() +
  theme(axis.text.x = element_text(size = 8, angle = 0), 
        plot.title = element_text(size = 13))
...
First we use stacked bar chart to explore the relationship between frequency of ordering takeout food and choice of transportation. The preliminary result is quite convincing as it's consistent with our experience and intuition. For the convenience of comparison, we aggregate the level of receving delivery "daily", "most days" and "several times a week" into one group "a few times a week or more" and combine "a few times a year","never" into "seldom or never".

It can be concluded from the bar chart that people who seldom or never receive takeout food take up the majority of people who choose walking as a way to travel around the city, when people order food delivery service a few times a week or more show a least relative tendency to walk. Similar situation happens in the "For-hire vehicle" group. Those who seldom or never receive food delivery has the lowest relative proportion in "For-hire vehicle" group when those who order a few times a week or more takeing up the most compared to other transportations.
...
Now we can observed the association more clearly. People with a higher tendency to use for-hired vehicle are also willing to choose takout dilivery service. That's probably a reason why Uber also develop Uber Eats as the customers are somehow overlapped.
...
## How do Locations Affect Choises of Transportations?

Transportation is naturally a topic highly related to space and geography elements. In this part, we extract the transportation of highest proportion in people's choices for each Zipcode and derive the "favourtie transportation by Zipcode" variable. 

From the choropleth, we can see that a majority of blocks in New York City choose Subway as their favourate way of transportation, which include most areas of Queens and Brooklyn, Manhattan Uptown and the South Bronx. From the New York City subway system map we can see these are also the areas where subway lines are constructed in high density. 

As there are not so many subway lines in Staten Island, people use car as a major way of traveling, which is also the case for some areas in Bronx and Long Island as they are far from the center of NYC, so car is needed for commute and mobility.

Interestingly, people living in downtown of Manhattan and some parts of Brooklyn choose walking as their favourite way of transportation. It may be realted to the density of commerial activities so that people do not have to travel long distance to meet their daily needs. As they live in the center of the city, it's unnecessary to take subway or drive a car to their workplaces.
...
ggplot(plot_borough, aes(reorder(borough, - prop), prop)) +
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(x = "Borough", y = "Proportion", 
       title = "The Distribution of Repondents in Different Boroughs",
       caption = "Source: https://data.cityofnewyork.us/Transportation/Citywide-Mobility-Survey-Main-Survey/dd6w-hnq9") +
  theme_economist()+
  theme(axis.text.x = element_text(size = 8, angle = 0), 
        legend.position = "right", 
...
for (i in c(1:nrow(df_like))){
  df_like[['favourite']][i] <- unlist(strsplit(df_like[['transportation']][i],
                                               split = "_"))[1]
}
```
```{r}
...
zip_choropleth(plot_fav, county_zoom = nyc_fips,
               title = "Favourite Transportation by Zipcode",
               num_colors = 5)+
  scale_fill_brewer(na.value = "white",
                    guide_legend(title = "Favourite Transportation by Zipcode"))+
  scale_fill_manual(values = c("#4a4a4a", "#8abbd0", "purple", "#efe8d1", "#00008b"))
```

As a result of difference in infrastructure construction and urban area function, people in different boroughs and locations of the city form different preference for transportations.

From a environmental perspective, we draw the choropleth for the proportion of people choosing a low-carbon way of traveling. In our definition, those who take subway and other public transportation service as well as people who choose walking and riding bicycle will all be considered as the environment-friendly group in term of traveling.
...
app_usage_plot <- ggplot(app_usage, 
                  ...
                  labs(x = 'App Name', y = 'Percentage',
                       title = "Ride Hail Services App Usage") +
...
purpose_plot <- ggplot(purposes, aes(x = fct_reorder(purpose, 
                ...
                labs(x = 'Purposes', y = 'Percentage',
                     title = "Purposes of Using Ride Hail Services",
                     caption = "Source: https://data.cityofnewyork.us/Transportation/Citywide-Mobility-Survey-Main-Survey/dd6w-hnq9") +
                theme_economist() +
...
bin_freq_plot <- ggplot(data = bin_freq, 
  ...
  labs(x = 'Usage times', y = 'Percentage',
       title = "Ride-hail Service\nCoverage") +
  theme_economist() +
  theme(axis.text.x = element_text(size = 7, angle = 20, vjust = 0.5),
        plot.title = element_text(size = 10),
        legend.position = "right", 
        legend.title = element_blank(),
        legend.text = element_text(size = 9),
