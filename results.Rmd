...
survey_3 <- merge(travel_mode, survey, by = "UniqueID", all.x = TRUE) %>%
  filter(Bicycle == 0) %>%  # Only forcus on those did not choose bicycle
...
ggplot(survey_3) +
   geom_mosaic(aes(x = product(Safe, Gender), fill = Gender, weight = Count), offset = 0.025) + 
   labs(x = 'Gender', y = 'Safe', title = "Proportion of Males and Females with Different Answers",
        caption = "Source: https://data.cityofnewyork.us/Transportation/Citywide-Mobility-Survey-Main-Survey/dd6w-hnq9") +
